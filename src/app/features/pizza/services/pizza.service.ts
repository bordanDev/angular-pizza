...
import { FiltrationUI, Interval } from '../interfaces/filtration.interface';
...
export class PizzaService {
  ...
  private pizzaTagsSubject: BehaviorSubject<CheckboxInterface[]> =
    new BehaviorSubject<CheckboxInterface[]>([]);
  pizzaTags$: Observable<CheckboxInterface[]> =
    this.pizzaTagsSubject.asObservable();

  public loadPizza(): Observable<Pizza[]> {
    return this.http
      .get<Pizza[]>(MockServerEnum.MOCK + '/' + EndpointsEnum.PIZZA)
      .pipe(
        tap((pizzas) => {
  ...
  public loadUi(): Observable<FiltrationUI> {
    return this.http
      .get<FiltrationUI>(MockServerEnum.MOCK + '/' + EndpointsEnum.FILTRATION)
      .pipe(
        tap((ui) => {
  ...
  private pizzaAdditionalIngredientsSubject: BehaviorSubject<
    PizzaAdditionalIngredient[]
  > = new BehaviorSubject<PizzaAdditionalIngredient[]>([]);
  public pizzaAdditionalIngredients$: Observable<PizzaAdditionalIngredient[]> =
    this.pizzaAdditionalIngredientsSubject.asObservable();

  public loadAdditionalIngredUi(): Observable<PizzaAdditionalIngredient[]> {
    return this.http
      .get<
        PizzaAdditionalIngredient[]
      >(MockServerEnum.MOCK + '/' + EndpointsEnum.ADDITIONAL_INGRED)
      .pipe(
        tap((selectors) => {
          this.pizzaAdditionalIngredientsSubject.next(selectors);
          console.log();
  ...
  private doughTypesSubject: BehaviorSubject<RadioOptions[]> =
    new BehaviorSubject<RadioOptions[]>([]);
  doughTypes$: Observable<RadioOptions[]> =
    this.doughTypesSubject.asObservable();

  private checkboxesIngredsSubject: BehaviorSubject<CheckboxInterface[]> =
    new BehaviorSubject<CheckboxInterface[]>([]);
  checkboxesIngreds$: Observable<CheckboxInterface[]> =
    this.checkboxesIngredsSubject.asObservable();
  ...
  public setPizzaIngredients(ingredients: string[]) {
    ...
    const filtered = allPizzas.filter((pizza) =>
      pizza.ingredients.some((ing) => ingredients.includes(ing)),
    );
    console.log(filtered);
    this.filteredPizza.set(filtered);
  }

  public readonly filteredPizza = signal<Pizza[]>(
    this.pizzasSubject.getValue(),
  );
  ...
  public setFilteredPricePizzas(intervalPrice: Interval[]) {
    ...
    const filtered = allPizzas.filter(
      (pizza) =>
        pizza.price <= intervalPrice[0]?.max &&
        pizza.price >= intervalPrice[0]?.min,
    );
    this.filteredPizza.set(filtered);
  }
